import { getCreatorBySlug, getAllCreators } from "@/lib/api";
import { RepoCard } from "@/components/repo-card";
import { SkillCard } from "@/components/skill-card";

export async function generateStaticParams() {
  try {
    const creators = await getAllCreators({ limit: 100 });
...
export default async function CreatorDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { repos, skills, ...creator } = data;
  return (
    <div className="mx-auto max-w-7xl px-6 py-14">
      <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-8">
        ...
        <p className="mt-5 max-w-3xl text-white/70">{creator.bio}</p>
        <div className="mt-6 flex flex-wrap gap-2">
          {creator.tags.map((tag) => <span key={tag} className="rounded-full border border-white/10 px-3 py-1 text-xs text-white/60">{tag}</span>)}
        </div>
      </div>
