...
export const stats = {
  ...
  totalDownloads: skills.reduce((sum, item) => sum + item.downloads, 0),
};

export function getCreators() { return creators; }
export function getCreatorBySlug(slug: string) { return creators.find((item) => item.slug === slug); }
export function getRepos() { return repos; }
export function getRepoBySlug(slug: string) { return repos.find((item) => item.slug === slug); }
export function getSkills() { return skills; }
export function getSkillBySlug(slug: string) { return skills.find((item) => item.slug === slug); }
export function getReposByCreator(creatorId: string) { return repos.filter((item) => item.creatorId === creatorId); }
export function getSkillsByCreator(creatorId: string) { return skills.filter((item) => item.creatorId === creatorId); }
export function getSkillsByRepo(repoId: string) { return skills.filter((item) => item.repoId === repoId); }
export function searchAll(query: string) {
  const q = query.toLowerCase();
  return {
    creators: creators.filter((item) => item.name.toLowerCase().includes(q) || item.handle.toLowerCase().includes(q)),
    repos: repos.filter((item) => item.name.toLowerCase().includes(q) || item.description.toLowerCase().includes(q)),
    skills: skills.filter((item) => item.name.toLowerCase().includes(q) || item.description.toLowerCase().includes(q) || item.tags.some((tag: string) => tag.includes(q))),
  };
}
