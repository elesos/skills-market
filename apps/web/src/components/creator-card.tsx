...
export function CreatorCard({ creator, locale }: { creator: Creator; locale: string }) {
  return (
    <a href={`/${locale}/creators/${creator.slug}`} className="group rounded-3xl border border-white/10 bg-white/5 p-5 transition hover:-translate-y-1 hover:border-cyan-400/40 hover:bg-white/7">
      ...
      <p className="mt-4 line-clamp-3 text-sm text-white/70">{locale === "zh" ? creator.bio_zh || creator.bio : creator.bio}</p>
